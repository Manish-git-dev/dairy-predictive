...
function buildAnomaly({ metric, type, value, expectedMin, expectedMax, expectedLabel, stats, entity, detectedAt, reason, action }) {
  ...
  const isOpenUpperBound = expectedMax >= Number.MAX_SAFE_INTEGER / 2;
  const halfRange = isOpenUpperBound
    ? Math.max(Math.abs(expectedMin) * 0.5, 0.01)
    : Math.max((expectedMax - expectedMin) / 2, Math.abs(center) * 0.05, 0.01);
  ...
  const expected = isOpenUpperBound ? Number(expectedMin) : round((expectedMin + expectedMax) / 2);
  return {
    anomalyId,
    ...
    expectedRange: { min: expectedMin, max: isOpenUpperBound ? null : expectedMax, label: expectedLabel },
...
const anomalyService = {
  detect: async (data = {}, organizationId) => {
    ...
    lots.forEach((lot) => {
      ...
      if (finite(lot.quantityLitres) && date) add(buildAnomaly({ metric: 'milk_volume', type: 'volume_anomaly', value: lot.quantityLitres, expectedMin: round(volumeMin), expectedMax: round(volumeMax), expectedLabel: `${round(volumeMin)}–${round(volumeMax)} L`, stats: volumeStats, entity: { type: 'MilkLot', id: lot._id, label: lot.lotId, organization: organizationId }, detectedAt: date, reason: `Milk lot ${lot.lotId} volume is outside the observed operating range.`, action: 'Review the collection record and confirm weighing/measurement at the collection centre.' }));
      if (finite(lot.temperature) && date) add(buildAnomaly({ metric: 'temperature', type: 'temperature_alert', value: lot.temperature, expectedMin: 2, expectedMax: 8, expectedLabel: '2–8 °C', stats: statistics(lots.map((item) => item.temperature)), entity: { type: 'MilkLot', id: lot._id, label: lot.lotId, organization: organizationId }, detectedAt: date, reason: `Milk lot ${lot.lotId} temperature is outside the configured cold-chain range.`, action: 'Inspect chilling equipment, transport conditions and temperature logs for this lot.' }));
    });
    ...
    tests.forEach((test) => {
      ...
      if (!date) return;
      add(buildAnomaly({ metric: 'fat', type: 'quality_deviation', value: test.parameters?.fat, expectedMin: 2, expectedMax: 7, expectedLabel: '2–7 %', stats: fatStats, entity: { type: 'QualityTest', id: test._id, label: test.testId, organization: organizationId }, detectedAt: date, reason: `Quality test ${test.testId} has an abnormal fat reading.`, action: 'Review the sample, collection source and calibration of the fat testing equipment.' }));
      add(buildAnomaly({ metric: 'snf', type: 'quality_deviation', value: test.parameters?.snf, expectedMin: 6, expectedMax: 10, expectedLabel: '6–10 %', stats: snfStats, entity: { type: 'QualityTest', id: test._id, label: test.testId, organization: organizationId }, detectedAt: date, reason: `Quality test ${test.testId} has an abnormal SNF reading.`, action: 'Review the sample and investigate recurring SNF deviations at the source centre.' }));
    });
    ...
    daily.forEach((entry, key) => {
      ...
      if (rate > threshold) {
        ...
        add(buildAnomaly({ metric: 'rejection_rate', type: 'rejection_spike', value: round(rate * 100), expectedMin: 0, expectedMax: round(threshold * 100), expectedLabel: `0–${round(threshold * 100)} %`, stats: null, entity: { type: 'OperationsDay', label: key, organization: organizationId }, detectedAt: date, reason: `Daily rejection rate reached ${round(rate * 100)}% across ${entry.total} milk lots.`, action: 'Review rejected lots, quality-test results and collection-centre patterns for this day.' }));
      }
    });
    ...
    inventory.forEach((item) => {
      ...
      if (Number(item.quantity) < Number(item.minimumStock)) {
        ...
        add(buildAnomaly({ metric: 'inventory', type: 'inventory_anomaly', value: item.quantity, expectedMin: Number(item.minimumStock), expectedMax: Number.MAX_SAFE_INTEGER, expectedLabel: `at least ${Number(item.minimumStock)} ${item.unit || 'units'}`, stats: null, entity: { type: 'Inventory', id: item._id, label: item.location || 'Inventory item', organization: organizationId }, detectedAt: date, reason: 'Inventory quantity is below its configured minimum stock level.', action: 'Review consumption and initiate replenishment according to the reorder policy.' }));
      }
    });
    ...
    payments.forEach((payment) => {
      if (!finite(payment.netAmount) || Number(payment.netAmount) < 0) {
        ...
        add(buildAnomaly({ metric: 'payment', type: 'payment_anomaly', value: Number(payment.netAmount) || 0, expectedMin: 0, expectedMax: Number.MAX_SAFE_INTEGER, expectedLabel: 'non-negative net amount', stats: null, entity: { type: 'Payment', id: payment._id, label: payment.paymentId, organization: organizationId }, detectedAt: date, reason: `Payment ${payment.paymentId} contains an invalid negative or non-numeric net amount.`, action: 'Review the payment calculation and approval trail before disbursement.' }));
      }
    });
    ...
    for (const anomaly of anomalies) {
      const result = await AnomalyEvent.findOneAndUpdate({ anomalyId: anomaly.anomalyId, organization: organizationId }, { $set: anomaly }, { upsert: true, new: true, setDefaultsOnInsert: true });
      saved.push(result);
    }
  ...
  getAll: async (organizationId, filters = {}) => {
    ...
    if (search) query.$or = [{ metric: { $regex: search, $options: 'i' } }, { type: { $regex: search, $options: 'i' } }, { description: { $regex: search, $options: 'i' } }, { 'entity.label': { $regex: search, $options: 'i' } }];
    const [items, total] = await Promise.all([AnomalyEvent.find(query).sort({ detectedAt: -1 }).skip(skip).limit(limitNum).lean(), AnomalyEvent.countDocuments(query)]);
  ...
  updateStatus: async (id, status, resolution, userId, organizationId) => {
    ...
    if (status === 'resolved') { update.resolvedBy = userId; update.resolvedAt = new Date(); }
    if (status === 'investigating') { update.resolvedBy = undefined; update.resolvedAt = undefined; }
  ...
  getRiskScores: async (organizationId) => {
    ...
    events.forEach((event) => { if (aggregated[event.type] !== undefined) aggregated[event.type] += Number(event.riskScore) || 0; aggregated.total += Number(event.riskScore) || 0; });
    return aggregated;
  },
  ...
  explainAnomaly: async (id, organizationId) => {
    const event = await AnomalyEvent.findOne({ _id: id, organization: organizationId }).lean();
    if (!event) throw new ApiError(404, 'Anomaly not found');
    return { anomalyId: event.anomalyId, metric: event.metric, riskScore: event.riskScore, explanation: event.explanation || `Detected ${event.metric} anomaly with risk score ${event.riskScore}.`, factors: event.factors || [], recommendation: event.recommendedAction || `Investigate ${event.metric} for ${event.entity?.label || event.entity?.type || 'the affected entity'}.` };
  }
};
