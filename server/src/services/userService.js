...
const userService = {
  ...
  getAll: async (organizationId, filters) => {
    ...
    const items = await User.find(query).select('-password').skip(skip).limit(limitNum);
    const total = await User.countDocuments(query);
    return { items, total, page, limit: limitNum, totalPages: Math.ceil(total / limitNum) };
  },

  getById: async (id, organizationId) => {
    return await User.findOne({ _id: id, organization: organizationId }).select('-password');
  },

  update: async (id, data, organizationId) => {
    // Prevent password update via this generic method
    delete data.password;
    return await User.findOneAndUpdate({ _id: id, organization: organizationId }, data, { new: true }).select('-password');
  },
  ...
  deactivate: async (id, organizationId) => {
    return await User.findOneAndUpdate({ _id: id, organization: organizationId }, { isActive: false }, { new: true }).select('-password');
  },

  activate: async (id, organizationId) => {
    return await User.findOneAndUpdate({ _id: id, organization: organizationId }, { isActive: true }, { new: true }).select('-password');
  }
};
