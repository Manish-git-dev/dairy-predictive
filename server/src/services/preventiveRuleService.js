...
const getMetricValue = async (metric, organizationId, windowHours = 24) => {
  ...
  switch (metric) {
    ...
    case 'collection_volume': {
      ...
      return { value: rows.length ? rows.reduce((sum, row) => sum + (Number.isFinite(row.quantityLitres) ? row.quantityLitres : 0), 0) : null, unit: 'L', sampleSize: rows.length };
    }
    case 'average_fat':
    case 'average_snf': {
      ...
      return { value: finiteAverage(rows.map(row => Number(row.parameters && row.parameters[key]))), unit: '%', sampleSize: rows.length };
    }
    case 'temperature': {
      const rows = await MilkLot.find({ organization: organizationId, createdAt: { $gte: since } }).select('temperature').lean();
      return { value: finiteAverage(rows.map(row => Number(row.temperature))), unit: '°C', sampleSize: rows.length };
    }
    ...
    case 'capacity_utilization': {
      ...
      const values = rows.map(row => Number(row.capacityLitres) > 0 ? (Number(row.currentLoad) / Number(row.capacityLitres)) * 100 : null).filter(Number.isFinite);
...
const trigger = async (id, requesterId, organizationId) => {
  ...
  const approval = await Approval.create({
    ...
    status: 'pending',
    aiRecommendation: { action: rule.action, confidence: 1, reasoning: `${rule.metric} evaluated at ${result.value}${result.unit} against ${result.operator} ${result.threshold}.`, modelVersion: 'rule-engine-1.0.0' },
  ...
  await notificationService.notify(
    requesterId,
    'preventive_action',
    `Preventive action triggered: ${rule.name}`,
    `Condition met at ${result.value}${result.unit}. Approval ${approval.approvalId} is pending review.`,
    { type: 'PreventiveRule', id: rule._id },
    organizationId,
